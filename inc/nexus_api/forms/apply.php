<?php include(nexus_plugin_inc('nexus_api/data/nexus_options.php')); // Nexus Overall Options ?>
<?php include(nexus_plugin_inc('nexus_api/data/current_user.php')); // Current User Details ?>
<?php
    if ($cuTYPE == 'designer' && trim(get_post_meta($postID, 'nexus_assigned_user', true)) == '') { 
?>

<div class="pure-g fullWidth">

    <div class="pure-u-5-5 tbs requestHeader">
        <h1 class="newRequestTitle txtWhite"><img src="data:image/png;base64," />Apply for Job "<?php echo get_the_title($postID); ?>"</h1> 
    </div>

</div>

<div class="pure-g">

    <div class="pure-u-5-5" id="applicationForm">

        <div class="pure-u-5-5">&nbsp;</div>

        <form name="application" id="application" class="nexus_ajaxForm" data-function="nexus_applyForJob">
            <div class="pure-g">
                <div class="pure-u-5-5 pure-u-md-16-24 leftFormContent">
                    <div class="form-style-7 TEST">
                        <label for="applicationDetails">Application details:</label>
                        <textarea name="applicationDetails"></textarea>
                        <span class="errorText">Please add a quote or job description</span>
                    </div>
                </div>
                <div class="pure-u-5-5 pure-u-md-8-24 rightFormContent" style="padding-top:15px;">
                    <div class="pure-g">
                        <div class="pure-u-5-5">
                            <input type="hidden" name="projectID" value="<?php echo $postID; ?>" />
                            <input type="hidden" name="userID" value="<?php echo $cuID; ?>" />
                            <a href="#application" class="cta nexus_ajaxSubmit green txtC" style="margin-top:7px;">APPLY</a>
                        </div> 
                    </div> 
                </div>
            </div> 
        </form>
    </div>
    
</div>
<?php } ?>