...
<?php if ($requestTypes) { foreach ($requestTypes as $type) { ?>
    ...
    <div class="pure-u-5-5 pure-u-sm-1-2 pure-u-md-1-3 menuItem">
        ...
        <?php if (trim(get_post_meta($postID, 'nexus_assigned_user', true)) != '') { $stage = $stage; } else { $stage = 0; } ?>        
        
        <a class="menuButton ajaxFunction" data-query="single_request" data-status="<?php echo $stage; ?>" data-postid="<?php echo $postID; ?>" data-menu="single_request" data-tab="single_request" data-newtype="<?php echo $type; ?>" data-requesttype="<?php echo $type; ?>" href="#"></a>
    </div>
<?php } } ?>
</div>