<?php
    ...
    foreach ($postTypeArray as $postType) {
        ...
        if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); 
            ...
            if ($rate) { $rates[$ratesArr][] = intval($rate, 10); }
        ...
        if (count($rates[$ratesArr]) > 0) { $$ratesArrAv = round(array_sum($rates[$ratesArr]) / count($rates[$ratesArr])); } else { $$ratesArrAv = 0; }
        ...
        foreach ($adminUsers as $adminUser) {
            ...
            if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); $rate = get_post_meta(get_the_ID(),'nexus_request_rating',true); if ($rate) { $srate[$ratings][] = intval($rate, 10); $fullRatings[] = intval($rate, 10); } endwhile; endif; wp_reset_query();

            $$rfound = $query->found_posts; $rFoundArr[] = $$rfound; 
            $subCount[$postLabel] = $$rfound;

            if (count($srate[$ratings]) > 0) { $$average = round( array_sum($srate[$ratings]) / count($srate[$ratings]) ); } else { $$average = 0; }
