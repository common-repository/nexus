<script>
    ...
    function nexus_ajaxMe(action, postData, returnFunction, returnFunctionType) {
        ...
        //console.log(returnFunctionType);
        jQuery.ajax({
    ...
    function nexus_Chat(userID, text, ts, postid,file,fileType) {
            var blank = '<div class="pure-u-4-24 txtC">&nbsp;</div>',
                userDiv = '<div class="pure-u-18-24 pure-u-md-20-24 userComment">',
                content = '<div class="commentBubble"><span class="dont-break-out animatedFast slideInRight">'+text+'</span></div></div>',
                output = blank+userDiv+content;
            
            if (file) {
                var filename = (file.includes('\\') ? file.match(/.*\\(.*)$/)[1] : file.match(/.*\/(.*)$/)[1]),
                    fileContent = '<div class="commentBubble"><span class="dont-break-out animatedFast slideInRight">'+filename+'</span></div></div>',
                    imgContent = '<div class="commentBubble"><span class="dont-break-out animatedFast slideInRight"><a class="imgPopup" href="'+file+'"><img src="'+file+'" class="chatImgPreview" /></a></span></div></div>',
                    fileOutput = blank+userDiv+fileContent,
                    imgOutput = blank+userDiv+imgContent,
                    fileEncode = encodeURIComponent(file);
            } else {
                var filename = false,
                    fileContent = false,
                    imgContent = false,
            ...
            jQuery.ajax({
                url: nexus_ajaxurl+'?action=nexus_chatLogUpdate',
                type: 'post',
                data: {userID: userID, text: text, timestamp: ts, id: postid,file: fileEncode, fileType: fileType, wp:file},
                success: function(data) {
            ...
            jQuery('#chatInner > .pure-g > #noMessage').remove();
            if (text) { jQuery('#chatInner > .pure-g').append(output); } 
            if (filename) {
                if (file.includes('.png') || file.includes('.jpg')) {
                    //console.log(file);
                    jQuery('#chatInner > .pure-g').append(imgOutput); 
                } else {
                    jQuery('#chatInner > .pure-g').append(fileOutput);
                }
            }
            
            var chat = jQuery('#chatInner');
            chat[0].scrollTop = chat[0].scrollHeight;
            
            jQuery('input[name="nexus_chatlog"]').val('');
            jQuery('input[name="nexus_chatFile"]').val('');
        }
    ...
    function nexus_removeBinders() {         
        ...
        jQuery(document).off('keyup','input[name="nexus_chatlog"]');
        jQuery(document).off('click','.nexus_ChatlogSubmit');
        jQuery(document).off('click', '.nexusUploadBTN');
        jQuery(document).off('change', '#userImage');
    }
    ...
    function nexus_init() {    
        ...
        jQuery(document).on('click', '.nexusUploadBTN', function(e) {
            e.preventDefault();
            jQuery('#userImage').click();
        });     
        
        function nexus_previewIMG(input, id) {
        ...
        jQuery(document).on('change', '#userImage', function(e) {
            e.preventDefault();   
            nexus_previewIMG(this, '.profileIMG');
        }); 
        
        if (jQuery(document).find('#chatInner').length) {
            var chat = jQuery('#chatInner');
            chat[0].scrollTop = chat[0].scrollHeight;
        }
        
        var nexus_chatfiles = wp.media({
            title: 'Upload File',
            multiple: false
        });
        ...
        jQuery('#chatInner').magnificPopup({
            delegate: 'a.imgPopup',
            type: 'image',
            gallery: {
                enabled: true
        ...
        jQuery(document).on('keyup','input[name="nexus_chatlog"]',function(e){
                //console.log(e);
            if (e.keyCode == 13) {
                //console.log('enter');
                ...
                var userID = jQuery(this).attr('data-user'),
                    ...
                    file = jQuery('input[name="nexus_chatFile"]').val(),
                    fileEsc = file,
                    fileType = jQuery('input[name="nexus_chatFile"]').attr('data-filetype'),
                    ts = Date.now(),
                    postid = jQuery(this).attr('data-post');

                if (file || text) {
                    nexus_Chat(userID,text,ts,postid,fileEsc,fileType);
                }
                jQuery('input[name="nexus_chatFile"]').attr('data-filetype','');
            }
        });

        jQuery(document).on('click', '.nexus_chatlogSubmit',function(e){
            var userID = jQuery(this).attr('data-user'),
                text = jQuery('input[name="nexus_chatlog"]').val(),
                file = jQuery('input[name="nexus_chatFile"]').val(),
                fileEsc = file,
                fileType = jQuery('input[name="nexus_chatFile"]').attr('data-filetype'),
                ts = Date.now(),
    ...
    jQuery(document).ready(function ($) {
        ...
        $(document).on('click','.nexus_chatButton.chatMedia', function(e){
            e.preventDefault();
            var type = $(this).attr('data-type');
            nexus_chatfiles.open().on('select', function(e){
                var filesObject = nexus_chatfiles.state().get('selection'),
                    filesJSON = filesObject.toJSON(),
                    filesCount = Object.keys(filesJSON).length,
                    fileURL, fileIDList = [];
                ...
                $('input[name="nexus_chatFile"]').val(fileURL);
                $('input[name="nexus_chatFile"]').attr('data-filetype','wordpress');
                //$(document).find('.chatlogSubmit').click();
            });
        });
        ...
        // Onlick event to run ALL future pageload queries
            ...
            function nexus_ajaxFunction(dataArr) {
                ...
                $(document).find("#currentUserView").load(nexus_ajaxurl+"?action=nexus_currentUserView");
            ...
            $(document).on('keyup','input[name="nexus_jumpToJob"]',function(e){
                if(e.which == 13) {
                    var postid = parseInt($(this).val(),10),
                        dataArr = {menu:'single_request', query:'single_request', postid: postid};
                    
                    if ($(this).val() % 1 === 0) {
                        $.ajax({
                            url: nexus_ajaxurl+'?action=nexus_is_post',
                            type: 'post',
                            data: {postid: postid},
                            success: function(data) {
                                var dataArray = JSON.parse(data);
        ...
        // Form Submission - With Files
            $(document).on('submit', 'form.nexus_formDataFiles', function(e){
                e.preventDefault();
                var formData = new FormData($(this)[0]), formFunction = $(this).attr('data-function');
        ...
        // Validation of forms (checks for .required class and validates them - could be expanded upon for emails etc)
            function validateRequest(form) {
                ...
                $('label.red').removeClass('red');  $('select.red').removeClass('red'); $('.error').removeClass('error');
                $(form).find('.required').each(function(){
                    var theID = $(this).attr('name'), value = $(this).val();
                    if (!value || value == 'na') {
                        validated = false;
                        if (form != '#nexusSignupForm') { 
